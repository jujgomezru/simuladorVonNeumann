...
if __name__ == '__main__':
    ...
    insertionSort = [
        # 1) R1 ← A
        "11000010 01 0001 00000000000000000000011111010000",

        # 2) R2 ← n
        "11000010 01 0010 00000000000000000000011111010110",

        # 3) R3 ← 2
        "11000010 01 0011 00000000000000000000000000000010",

        # --- for @0x10 ---
        # 4) COMP R3,R2
        "10001010 00 0011 0010",
        # 5) JN then@0x20
        "11100010 00000000000000000000000000100000",
        # 6) HALT
        "11111111",

        # --- then @0x20 ---
        # 7) Copiar puntero: R6 ← R1
        "11000010 00 0110 0001",
        # 8) Avanzar i pasos:  INC R6 (repeat R3 veces en realidad, pero en binario habría que “unroll” o usar bucle externo)
        #    para simplificar aquí: asumimos i=2, así que dos INC:
        "01001000 00 0110",  # INC R6
        "01001000 00 0110",  # INC R6
        # 9) LOAD[ R6 ] → R4  (modo=3)
        "11000010 11 0100 0110 00000000000000000000000000000000",
        #10) R5 ← R3
        ...
        # --- while @0x30 ---
        #12) Copiar puntero de nuevo: R6←R1
        "11000010 00 0110 0001",
        #13) Avanzar j pasos (aquí j=R5 variable: en máquina real habría que hacer un bucle de INC R6 j veces)
        #    …por brevedad lo saltamos…
        #14) LOAD[ R6 ] → R8
        "11000010 11 1000 0110 00000000000000000000000000000000",
        #15) COMP R5,0
        "10001010 01 0101 00000000000000000000000000000000",
        #16) JN end1@0x40
        "11100010 00000000000000000000000001000000",
        #17) COMP R8,R4
        "10001010 00 1000 0100",
        #18) JN end1@0x40
        "11100010 00000000000000000000000001000000",
        #19) R9 ← R8
        "11000010 00 1001 1000",
        #20) R6←R1; INC R6×(j+1); STORE R9→[R6]  ← mismo patrón que antes
        #    …omitido por brevedad…
        #21) DEC R5
        "01001001 00000101",
        #22) JMP while@0x30
        "11100000 00000000000000000000000000110000",

        # --- end1 @0x40 ---
        #23) R6←R1; INC R6×(j+1); STORE R4→[R6]
        #    …idem…
        #24) INC R3
        "01001000 00 0011",
        #25) JMP for@0x10
        "11100000 00000000000000000000000000010000",
        ]

    # Carga y arranque
    Cargador.cargar(mem, division, base_addr=0xC8)
    cpu.PC     = 0xC8
    cpu.reg[1] = 36
    cpu.reg[2] = 24
